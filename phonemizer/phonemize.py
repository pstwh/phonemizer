...
def get_phonemizer(
    ...
    """Text to phonemes converter backend"""
    _check_arguments(
    ...
    if backend == "espeak":
        ...
        return BACKENDS[backend](language, logger=logger)
...
def phonemize(  # pylint: disable=too-many-arguments
    text,
    phonemizer: Optional[BaseBackend] = None,
    language: str = "en-us",
    backend: Backend = "espeak",
    separator: Optional[Separator] = default_separator,
    strip: bool = False,
    ...
    return _phonemize(
        phonemizer, text, separator, strip, njobs, prepend_text, preserve_empty_lines
    )
    ...
    if backend not in ("espeak", "espeak-mbrola", "festival", "segments"):
        raise RuntimeError(
            "{} is not a supported backend, choose in {}.".format(
                backend, ", ".join(("espeak", "espeak-mbrola", "festival", "segments"))
            )
        )
